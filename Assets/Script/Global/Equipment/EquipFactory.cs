...
internal static class EquipFactory
{
    ...
    internal static Equipment LotteryEquipment(MapRewardData rewardInfo, RewardRank rank, int lastlevel)
    {
        int resultGrade = LotteryGrade(rank); // = 로터리;
        Random r = new Random();
        int itemlevel = lastlevel;
        //int itemlevel = (Math.Max(1,(lastlevel / 10) - r.Next(0,3))) * 10; // Math.Max(50, level); //
        //int itemlevel = Math.Max(level - r.Next(0, 51), 1);//(level / 10) * 10; // Math.Max(50, level); //
        //10단위로짜르며, 최소 50레벨이후부터 환생할수있게한다.
        ...
        Equipment newOne = new Equipment(eData, sData, itemlevel, resultGrade);
        return newOne;
    }
    ...
    {
        // 노멀만드는데 1, 매직만드는데 드는비용 (1+3)+1 = 5, 레어만드는데 드는비용(5+6)+5 = 16
        int rand = UnityEngine.Random.Range(0, 101); // 5+16+80 0~100
        //if (rand < 1) return 3;
        if (rand < 5) return 2;
        if (rand < 5 + 16) return 1;
        return 0;
