    ...
    internal class RewardManager
    {
        ...
        internal void Init()
        {
            if (CurrentResult == null)
                RecordResult("Map01", true, 10, 3333, 200, 6);
            WaitingTime = 1.0f;
        ...
        internal void RecordResult(string mapID, bool clear, int floor, int clearTime, int totalElementLevel, int FinishResearchCount)
        {
            bool isFirstClear = false;
            if (clear)
                isFirstClear = Singleton<PlayerStatus>.getInstance().CheckFirstClear(mapID);

            CurrentResult = new ResultReward(mapID, clear, isFirstClear,
